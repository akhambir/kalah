package com.bb.akhambir.kalah.game.library.logic;

import com.bb.akhambir.kalah.game.library.logic.interfaces.KalahBoard;
...
public class KalahBoardImpl implements KalahBoard{
    ...
    public void turnFrom(Player player, int houseIndex) {
        ...
        if (houseIndex != PLAYER_ONE_STORAGE_INDEX && houseIndex != PLAYER_TWO_STORAGE_INDEX) {
            ...
            while (numberOfSeeds > 0) {
                ...
                if (isLastSeed(numberOfSeeds) && isYourStorage(houseToIncrementIndex, player)) {
                    turnAgain(player);
                }
    ...
    private void takeOpponentsSeeds(int houseToIncrementIndex, Player player) {
        ...
        if (player == Player.PLAYER_ONE) {
            boardState.set(PLAYER_ONE_STORAGE_INDEX, opponentsSeeds);
        } else if (player == Player.PLAYER_TWO) {
            boardState.set(PLAYER_TWO_STORAGE_INDEX, opponentsSeeds);
        }
    }
    ...
    private void turnAgain(Player player) {
        if (player == Player.PLAYER_TWO) {
            lastPlayer = Player.PLAYER_ONE;
        } else {
            lastPlayer = Player.PLAYER_TWO;
        }
    }
